class ProductList {
    constructor(productsUrl, renderContainer, cart) {
        this.cart = cart;
        fetch(productsUrl).then(result=>result.json()).then(products=>{
    ...
    addEventListeners() {
        $('#productInfoModal').on('show.bs.modal', event=>{
            ...
            modal.find('.modal-body .card-img-top').attr('src', 'img/products/' + product.image).attr('alt', product.title);
            modal.find('.modal-body .card-title').text(product.title);
            modal.find('button.buy').text(`${product.price} - Buy`).data('id', id);
        }
        );
